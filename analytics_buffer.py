...
class AnalyticsBuffer:
    ...
    def __init__(
        ...
        api_callable: Optional[Callable] 
        ):
        ...
        with self.lock:
            ...
            logger.debug(f"Event tracked. Buffer size: {len(self.buffer)}/{self.max_items}")
            ...
            if len(self.buffer) >= self.max_items:
                logger.info(f"Buffer full ({self.max_items} items) - triggering flush")
    ...
    def _flush(self):
        ...
        try:
            ...
            with self.lock:
                ...
                logger.info(f"Events preserved in buffer for retry. Buffer size: {len(self.buffer)}")
